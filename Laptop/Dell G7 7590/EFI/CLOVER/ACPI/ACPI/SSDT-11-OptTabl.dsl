 ...
 *
 * Disassembly of SSDT-11-OptTabl.aml, Fri Mar 20 04:26:39 2020
 *
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    External (_SB_.PCI0.PEG0.CMDR, UnknownObj)    // (from opcode)
            ...
            Method (_ON, 0, Serialized)  // _ON_: Power On
            {
                ...
                ElseIf (LGreater (OSYS, 0x07D9))
                {
                    ...
                    If (LEqual (VEID, 0x10DE))
                    {
                        ...
                        If (LEqual (GPU1, Zero))
                        {
                            If (LEqual (\_SB.GGIV (0x0302000E), One))
                            {
                                Store (One, NVAF)
                            }
                            Else
    ...
    {
        OperationRegion (DCMS, SystemIO, 0x72, 0x02)
        Field (DCMS, ByteAcc, NoLock, Preserve)
        {
            CIND,   8, 
            CDAT,   8
        }
        ...
        {
            If (LAnd (LNot (PWRS), LLess (CBA0, 0x0A)))
            {
                Store (Zero, ADCE)
            }
    ...
    Scope (\_SB.PCI0.PEG0.PEGP)
    {
        Method (GC6I, 0, Serialized)
        {
            ...
            If (LEqual (GPU1, Zero))
            {
                ...
                While (LNotEqual (\_SB.GGIV (0x03010010), One))
                {
                    Sleep (One)
                }
            }
        ...
        Method (GC6O, 0, Serialized)
        {
            ...
            If (LEqual (GPU1, Zero))
            {
                ...
                If (LEqual (\_SB.GGIV (0x03010010), One))
                {
                    \_SB.PCI0.SGPO (SGGP, Zero, 0x0301000F, Zero, One)
                    While (LNotEqual (\_SB.GGIV (0x03010010), Zero))
                    {
                        Sleep (One)
                    }

                    \_SB.PCI0.SGPO (SGGP, Zero, 0x0301000F, Zero, Zero)
                }
        ...
        {
            If (LEqual (\_SB.GGIV (0x03010010), Zero))
            {
