 ...
 *
 * Disassembly of SSDT-6.aml, Sat Mar 28 18:24:27 2020
 *
...
DefinitionBlock ("", "SSDT", 1, "OptRef", "OptTabl", 0x00001000)
{
    ...
    External (_SB_.PCI0.P0LS, FieldUnitObj)
    ...
    Scope (\_SB.PCI0.PEG0.PEGP)
    {
        ...
        {
            Store ("<<< GC6O >>>", Debug)
            While (LNotEqual (\_SB.GGIV (0x01010015), One))
            {
                Sleep (One)
            }

            Store (Zero, \_SB.PCI0.PEG0.LNKD)
            \_SB.PCI0.SGPO (SGGP, Zero, 0x01010014, Zero, One)
            While (LNotEqual (\_SB.GGIV (0x01010015), Zero))
            {
                Sleep (One)
            }

            \_SB.PCI0.SGPO (SGGP, Zero, 0x01010014, Zero, Zero)
        ...
        {
            If (LEqual (\_SB.GGIV (0x01010015), Zero))
            {
